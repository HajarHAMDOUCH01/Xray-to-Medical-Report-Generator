...
class BiomedCLIPEncoder(nn.Module):  
    def __init__(self, 
        ...
        # Don't try to load custom weights during initialization
        # This will be handled by the main model's from_pretrained method
        print("BiomedCLIP initialized with default weights. Fine-tuned weights will be loaded later.")
    ...
    def load_finetuned_weights(self, weights_path):
        """Load fine-tuned weights after initialization."""
        if weights_path and os.path.exists(weights_path):
            try:
                state_dict = torch.load(weights_path, map_location='cpu', weights_only=True)
                self.model.load_state_dict(state_dict, strict=False)
                print(f"✓ BiomedCLIP fine-tuned weights loaded from {weights_path}")
                return True
            except Exception as e:
                print(f"✗ Error loading BiomedCLIP fine-tuned weights: {e}")
                return False
        return False
