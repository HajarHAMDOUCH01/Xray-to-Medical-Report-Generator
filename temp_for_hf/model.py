...
class XrayReportGenerator(PreTrainedModel):
    ...
    def from_pretrained(
        ...
        **kwargs
        ...
        # Try to download all checkpoint files first
        ...
        try:
            ...
            logger.info(f"BiomedCLIP file found: {biomedclip_file}")
        except Exception as e:
            logger.warning(f"BiomedCLIP file not found: {e}")
        ...
        try:
            ...
            logger.info(f"BioGPT file found: {biogpt_file}")
        except Exception as e:
        ...
        if biomedclip_file:
            try:
                logger.info(f"Loading BiomedCLIP weights from {biomedclip_file}")
                biomedclip_state = torch.load(biomedclip_file, map_location='cpu', weights_only=True)
                
                # Handle potential DataParallel wrapper
                if any(key.startswith('module.') for key in biomedclip_state.keys()):
                    biomedclip_state = {key.replace('module.', ''): value for key, value in biomedclip_state.items()}
        ...
        if biogpt_file:
            try:
                logger.info(f"Loading BioGPT weights from {biogpt_file}")
                biogpt_state = torch.load(biogpt_file, map_location='cpu', weights_only=True)
                
                # Handle potential DataParallel wrapper
                if any(key.startswith('module.') for key in biogpt_state.keys()):
                    biogpt_state = {key.replace('module.', ''): value for key, value in biogpt_state.items()}
        ...
        if final_model_file:
            try:
                logger.info(f"Loading final model weights from {final_model_file}")
                state_dict = torch.load(final_model_file, map_location='cpu', weights_only=True)
